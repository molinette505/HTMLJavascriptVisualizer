...
export class Scope {
    ...
    define(name, kind, declared = true) { if (this.variables[name]) throw new Error(`Variable ${name} déjà déclarée`); const addr = "0x" + (Math.floor(Math.random()*0xFFFF)).toString(16).toUpperCase().padStart(3, '0'); this.variables[name] = { value: undefined, kind, addr, initialized: false, declared: Boolean(declared) }; return addr; }
